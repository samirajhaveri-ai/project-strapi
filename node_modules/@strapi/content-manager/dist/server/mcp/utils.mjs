 ...
 */ const sanitizeFormatShape = async (permissionChecker, uid, doc, opts)=>{
    const sanitized = await permissionChecker.sanitizeOutput(doc);
    const formatted = await formatDocumentWithMetadata(permissionChecker, uid, sanitized, opts);
    if (formatted.data === null || formatted.data === undefined) {
        return formatted;
    }
    const shapedData = await shapeRelationsForMcp(uid, formatted.data);
    return {
        ...formatted,
        data: shapedData
    };
...
export { describeTool, ok, sanitizeFormatShape, slugifyUidForMcpToolName };
