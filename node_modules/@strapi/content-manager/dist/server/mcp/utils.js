 ...
 */ const sanitizeFormatShape = async (permissionChecker, uid, doc, opts)=>{
    ...
    const formatted = await metadata.formatDocumentWithMetadata(permissionChecker, uid, sanitized, opts);
    if (formatted.data === null || formatted.data === undefined) {
        return formatted;
    }
    const shapedData = await shapeRelations.shapeRelationsForMcp(uid, formatted.data);
    return {
        ...formatted,
        data: shapedData
    };
...
exports.ok = ok;
exports.sanitizeFormatShape = sanitizeFormatShape;
