...
import { Modal, Flex, Button, Typography, Field, SingleSelect, SingleSelectOption } from '@strapi/design-system';
import { useIntl } from 'react-intl';
import { styled } from 'styled-components';
import { useGetFolderStructureQuery, useBulkMoveMutation, useGetFolderQuery } from '../../../services/folders.mjs';
...
import { hasUniformSource, uniformSourceFolderId } from '../../../utils/itemSource.mjs';
 ...
 */ const BulkMoveDialog = ({ open, onClose, items, onSuccess })=>{
    const { formatMessage } = useIntl();
    const { toggleNotification } = useNotification();
    const { data: folderStructure = [], isUninitialized: isStructureUninitialized, isLoading: isLoadingStructure, isError: isStructureError } = useGetFolderStructureQuery(undefined, {
        skip: !open
    });
    const [bulkMove, { isLoading: isMoving }] = useBulkMoveMutation();
    const fileIds = useMemo(()=>items.filter((item)=>item.kind === 'file').map((item)=>item.id), [
        items
    ]);
    const folderIds = useMemo(()=>items.filter((item)=>item.kind === 'folder').map((item)=>item.id), [
        items
    ]);
    ...
    const destinationOptions = useMemo(()=>flattenFolderStructure(folderStructure, new Set(folderIds)).filter((option)=>canDropItemOnFolder({
                items,
    ...
    const handleMove = async ()=>{
        ...
        const sourceName = !isSourceUniform ? null : sourceFolderId === null ? rootLabel : sourceFolder?.name ?? rootLabel;
