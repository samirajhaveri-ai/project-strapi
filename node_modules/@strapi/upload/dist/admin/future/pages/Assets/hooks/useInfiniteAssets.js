 ...
 */ const PageSubscriber = ({ queryArgs, page, onRefreshed })=>{
    const { currentData } = assets.useGetAssetsQuery({
        ...queryArgs,
        page,
        pageSize: PAGE_SIZE
    });
 ...
 */ const useInfiniteAssets = ({ folder = null, sort, search, filters, enabled = true } = {})=>{
    ...
    const { currentData, isLoading, isFetching, error } = assets.useGetAssetsQuery({
        ...queryArgs,
        page,
        pageSize: PAGE_SIZE
    }, {
    ...
    const handlePageRefreshed = React.useCallback((refreshedPage, results)=>{
        setAccumulated((prev)=>{
    ...
    const subscribers = React.createElement(React.Fragment, null, Array.from({
        length: Math.max(0, page - 1)
    }, (_, index)=>index + 1).map((earlierPage)=>React.createElement(PageSubscriber, {
    ...
    React.useEffect(()=>{
        ...
        const apiState = store.getState()[api.uploadApi.reducerPath];
        const removeQueryResult = api.uploadApi.internalActions.removeQueryResult;
    ...
    const assets$1 = React.useMemo(()=>isChangingList ? [] : flattenPages(accumulated.pages), [
        isChangingList,
        accumulated.pages
    ]);
