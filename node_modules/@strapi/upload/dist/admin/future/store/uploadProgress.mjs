 ...
 */ const selectMetadataProgress = createSelector((state)=>state.uploadProgress.files, (files)=>{
    ...
    const pending = files.filter((f)=>f.metadataStatus === undefined && (f.status === 'pending' || f.status === 'uploading')).length;
 ...
 */ const selectIsGeneratingMetadata = createSelector((state)=>state.uploadProgress.files, (files)=>{
 ...
 */ const selectMetadataOutcome = createSelector((state)=>state.uploadProgress.files, (files)=>{
    const started = files.filter((f)=>f.metadataStatus !== undefined);
    if (started.length === 0) return null;
...
const { openUploadProgress, setFileUploading, setFileProgress, setFileComplete, setFileError, setFileMetadataGenerating, setFileMetadataResult, addUploadErrors, closeUploadProgress, toggleMinimize, cancelUpload, setUploadFailed, retryCancelledFiles } = uploadProgressSlice.actions;
const uploadProgressReducer = uploadProgressSlice.reducer;

export { addUploadErrors, cancelUpload, closeUploadProgress, openUploadProgress, retryCancelledFiles, selectAggregateProgress, selectIsGeneratingMetadata, selectMetadataOutcome, selectMetadataProgress, setFileComplete, setFileError, setFileMetadataGenerating, setFileMetadataResult, setFileProgress, setFileUploading, setUploadFailed, toggleMinimize, uploadProgressReducer };
