...
var aiMetadata = require('./validation/admin/ai-metadata.js');
...
var adminFile = {
   ...
   *
   * @experimental
   */ async unstable_generateAIMetadata (ctx) {
        const { userAbility } = ctx.state;
        ...
        const { fileIds } = await aiMetadata.validateGenerateAIMetadataBody(body);
        const aiMetadataService = index.getService('aiMetadata');
        if (!await aiMetadataService.isEnabled()) {
            return ctx.badRequest('AI Metadata service is not enabled');
        }
