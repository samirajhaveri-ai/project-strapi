...
var adminUpload = {
    ...
    async replaceFile (ctx) {
        const { state: { userAbility, user }, query: { id }, request: { body, files: { files: filesInput } = {} } } = ctx;
        if (typeof id !== 'string') {
            throw new utils.errors.ValidationError('File id is required');
        }
        const uploadService = index.getService('upload');
   ...
   */ async unstable_uploadFromUrls (ctx) {
        ...
        const tmpWorkingDirectory = await fse__default.default.mkdtemp(path__default.default.join(os__default.default.tmpdir(), 'strapi-url-upload-'));
