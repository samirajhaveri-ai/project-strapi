 ...
 */ const SUPPORTED_IMAGE_TYPES = constants.AI_METADATA_SUPPORTED_IMAGE_TYPES;
const isSupportedImage = (file)=>constants.isAIMetadataSupportedMime(file.mime);
...
const createAIMetadataService = ({ strapi })=>{
     ...
     *
     * @experimental
     */ async generateForFiles (fileIds, user) {
            ...
            const filesById = new Map(files.map((file)=>[
                    file.id,
                    file
                ]));
            ...
            for (const imageChunk of chunk(images, constants.AI_METADATA_CHUNK_SIZE)){
                try {
            ...
            return normalisedIds.map((id)=>resultsById.get(id) ?? {
                    id,
                    status: 'error',
     ...
     */ async processFiles (files) {
            ...
            const createEmptyMetadataResults = ()=>Array.from({
                    length: files.length
                }, ()=>null);
